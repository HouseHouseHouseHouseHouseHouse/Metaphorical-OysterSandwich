...
\section{Standards}
    ...
    \begin{frame}{The OSI Model (Open Systems Interconnection)}
        \smartdiagramset{
            description font= \tiny,
            descriptive items y sep=1.1
        }
        \smartdiagram[priority descriptive diagram]{
            {\textbf{1. Physical} (Ethernet): responsible for encoding data onto the physical link},
            {\textbf{2. Data-Link} (MAC, LLC): provides data transfer across a link (local network)},
            {\textbf{3. Network} (IPv4): enables communication across multiple data links (wide network)},
            {\textbf{4. Transport} (UDP, TCP): provides abstractions for conveying arbitrary data},
            {\textbf{5. Session} (TCP): provides a "dialogue" model},
            {\textbf{6/7. Presentation/Application} (TFTP): highest-level communication protocol, specific to programs}
        }
    \end{frame}
